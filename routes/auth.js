...
router.post('/create-super-admin', [
  ...
  body('security_key')
    .equals(process.env.SUPER_ADMIN_SECURITY_KEY || 'NMSME_SUPER_ADMIN_2024')
    .withMessage('Invalid security key'),
  body('admin_credentials.title')
    .trim()
    .isLength({ min: 2, max: 100 })
    .withMessage('Title must be between 2 and 100 characters'),
  body('admin_credentials.institution')
    .trim()
    .isLength({ min: 2, max: 200 })
    .withMessage('Institution must be between 2 and 200 characters'),
  body('admin_credentials.department')
    .trim()
    .isLength({ min: 2, max: 100 })
  ...
  try {
    ...
    const adminProfile = await Admin.create({
      ...
      created_by: superAdminUser._id
    });

    // Generate JWT token
    const token = generateToken(superAdminUser._id);
    ...
    res.status(201).json({
      ...
      data: {
        token,
        user: superAdminUser.getPublicProfile(),
        admin_profile: {
          id: adminProfile._id,
          title: adminProfile.title,
          institution: adminProfile.institution,
    ...
    res.status(500).json({
      success: false,
      error: 'Server error during super admin creation'
    });
  }
});
...
router.post('/admin/login', [
  ...
  try {
    ...
    if (user.role === 'admin' || user.role === 'super_admin') {
      const Admin = require('../models/Admin');
      adminProfile = await Admin.findOne({ user_id: user._id });
    }
