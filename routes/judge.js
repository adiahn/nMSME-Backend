...
const { Judge, Application, ApplicationAssignment, Score, Notification, User, BusinessProfile } = require('../models');
...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const assignments = await ApplicationAssignment.find({ judge_id: judge._id })
      .populate('application_id', 'category business_description status submitted_at')
...
router.get('/assignments', async (req, res) => {
  try {
    ...
    const assignments = await ApplicationAssignment.find(query)
      .populate({
        path: 'application_id',
        select: 'category business_description status submitted_at documents',
        populate: [
    ...
    if (category) {
      filteredAssignments = assignments.filter(assignment => 
        assignment.application_id.category === category
      );
    }
...
router.get('/assignments/:assignmentId', async (req, res) => {
  try {
    ...
    // Get anonymized application data
    const anonymizedApplication = assignment.application_id.getAnonymizedSummary();
...
router.post('/assignments/:assignmentId/declare-conflict', [
  ...
  try {
    ...
    await Notification.createSystemNotification(
      'admin', // This would need to be the actual admin user ID
      'Conflict Declared',
      `Judge ${req.user.first_name} ${req.user.last_name} has declared a conflict for application ${assignment.application_id}. Reason: ${req.body.reason}`,
      'high'
    );
...
router.get('/profile', async (req, res) => {
  try {
    ...
    const recentActivity = await ApplicationAssignment.find({ judge_id: judge._id })
      .sort({ assigned_at: -1 })
      .limit(10)
      .populate('application_id', 'category status');
